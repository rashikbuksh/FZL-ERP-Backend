...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.buyer_uuid IS DISTINCT FROM NEW.buyer_uuid THEN
            ...
            VALUES (NEW.uuid, 'buyer_uuid', OLD.buyer_uuid::text, NEW.buyer_uuid::text, 'UPDATE', NEW.created_by, NOW());
        END IF;

        IF OLD.party_uuid IS DISTINCT FROM NEW.party_uuid THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'party_uuid', OLD.party_uuid::text, NEW.party_uuid::text, 'UPDATE', NEW.created_by, NOW());
        END IF;

        IF OLD.marketing_uuid IS DISTINCT FROM NEW.marketing_uuid THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'marketing_uuid', OLD.marketing_uuid::text, NEW.marketing_uuid::text, 'UPDATE', NEW.created_by, NOW());
        END IF;

        IF OLD.merchandiser_uuid IS DISTINCT FROM NEW.merchandiser_uuid THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'merchandiser_uuid', OLD.merchandiser_uuid::text, NEW.merchandiser_uuid::text, 'UPDATE', NEW.created_by, NOW());
        END IF;

        IF OLD.factory_uuid IS DISTINCT FROM NEW.factory_uuid THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'factory_uuid', OLD.factory_uuid::text, NEW.factory_uuid::text, 'UPDATE', NEW.created_by, NOW());
        END IF;

        IF OLD.is_sample IS DISTINCT FROM NEW.is_sample THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'is_sample', OLD.is_sample::text, NEW.is_sample::text, 'UPDATE', NEW.created_by, NOW());
        END IF;

        IF OLD.is_bill IS DISTINCT FROM NEW.is_bill THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'is_bill', OLD.is_bill::text, NEW.is_bill::text, 'UPDATE', NEW.created_by, NOW());
        END IF;
        ...
        IF OLD.remarks IS DISTINCT FROM NEW.remarks THEN
            INSERT INTO zipper.order_info_log (order_info_uuid, field_name, old_value, new_value, operation, changed_by, changed_at)
            VALUES (NEW.uuid, 'remarks', OLD.remarks, NEW.remarks, 'UPDATE', NEW.created_by, NOW());
        END IF;
