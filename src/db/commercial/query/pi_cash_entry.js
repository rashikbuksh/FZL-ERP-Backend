	...
	try {
		const query = sql`
				SELECT
					...
					CASE WHEN pe.thread_order_entry_uuid IS NULL THEN vodf.order_number ELSE concat('ST', CASE WHEN toi.is_sample = 1 THEN 'S' ELSE '' END, to_char(toi.created_at, 'YY'), '-', LPAD(toi.id::text, 4, '0')) END as order_number,
