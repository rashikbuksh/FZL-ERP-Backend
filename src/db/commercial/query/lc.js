...
export async function selectAll(req, res, next) {
	...
	try {
		...
		SELECT
			...
			lc.updated_at,
			lc.remarks,
			ARRAY_AGG(DISTINCT lc_entry.payment_date::date) as payment_date,
			ARRAY_AGG(DISTINCT lc_entry.ldbc_fdbc::date) as ldbc_fdbc,
			ARRAY_AGG(DISTINCT lc_entry.acceptance_date::date) as acceptance_date,
			ARRAY_AGG(DISTINCT lc_entry.maturity_date::date) as maturity_date,
			ARRAY_AGG(DISTINCT lc_entry.handover_date::date) as handover_date,
			ARRAY_AGG(DISTINCT lc_entry.receive_date::date) as receive_date,
			ARRAY_AGG(DISTINCT lc_entry.document_submission_date::date) as document_submission_date,
			ARRAY_AGG(DISTINCT lc_entry.bank_forward_date::date) as bank_forward_date,
			ARRAY_AGG(DISTINCT lc_entry.document_receive_date::date) as document_receive_date,
