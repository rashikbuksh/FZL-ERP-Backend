...
export async function selectByDate(req, res, next) {
	...
	const dataQuery = sql`
        SELECT
            ...
            zdb.slot,
            CONCAT('B', to_char(zdb.created_at, 'YY'), '-', (zdb.id::text)) AS batch_no,
					...
					SUM((
						CASE 
							WHEN vodf.order_type = 'tape' 
								THEN (tcr.top + tcr.bottom + be.quantity) / 100 / tcr.dyed_mtr_per_kg::float8
	...
	UNION 
		SELECT 
			...
			CONCAT(pm.name, '(', pm.min_capacity::float8, '-', pm.max_capacity::float8, ')') AS machine_name,
			tb.slot,
			CONCAT('TB', to_char(tb.created_at, 'YY'), '-', (tb.id::text)) AS batch_no,
