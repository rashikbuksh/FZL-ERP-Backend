...
export async function selectDocumentRcvLog(req, res, next) {
	...
	const query = sql`
        SELECT 
            ...
            WHERE
                lc.handover_date IS NOT NULL AND lc.document_receive_date IS NULL AND  ${start_date ? sql`lc.created_at BETWEEN ${start_date}::TIMESTAMP AND ${end_date}::TIMESTAMP + interval '23 hours 59 minutes 59 seconds'` : sql`1=1`}
            GROUP BY
