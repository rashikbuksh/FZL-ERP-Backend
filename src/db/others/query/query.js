			...
			filterCondition = sql`
				...
				AND order_info.uuid IN (
					...
					WHERE vodf.item_description != '---' 
					  ...
					  AND ${is_sample === 'true' ? sql`oe.quantity - (sfg.warehouse + sfg.delivered) > 0 AND sfg.dying_and_iron_prod > 0` : sql`oe.quantity - (sfg.warehouse + sfg.delivered) > 0 AND sfg.finishing_prod > 0`} 
