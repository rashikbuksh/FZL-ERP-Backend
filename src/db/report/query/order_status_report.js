	...
	try {
		...
		const query = sql`
            ...
            SELECT
                ...
                CASE WHEN dbm.batch_rank = 1 THEN oe.quantity::float8 ELSE '--' END as quantity,
                CASE WHEN dbm.batch_rank = 1 THEN oe.party_price::float8 ELSE '--' END as party_price,
                CASE WHEN dbm.batch_rank = 1 THEN oe.company_price::float8 ELSE '--' END as company_price,
                ...
                CASE WHEN dbm.batch_rank = 1 AND sfg.recipe_uuid IS NULL THEN oe.quantity::float8 ELSE '--' END as not_approved_quantity,
                CASE WHEN dbm.batch_rank = 1 AND sfg.recipe_uuid IS NOT NULL THEN oe.quantity::float8 ELSE '--' END as approved_quantity,
                sfg.recipe_uuid,
                recipe.name as recipe_name,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(oe.quantity, 0)::float8 ELSE '--' END as total_quantity,
                CASE 
                ...
                CASE WHEN dbm.batch_rank = 1 THEN sfg.delivered::float8 ELSE '--' END as delivered,
                CASE WHEN dbm.batch_rank = 1 THEN sfg.warehouse::float8 ELSE '--' END as warehouse,
                CASE WHEN dbm.batch_rank = 1 THEN (oe.quantity::float8 - sfg.delivered::float8) ELSE '--' END as balance_quantity,
                fo.order_type,
                fo.is_inch,
                CASE
                    ...
                    ELSE 'Cm'
                END as unit,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(sps.finishing_quantity, 0)::float8 ELSE '--' END as total_finishing_quantity,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(dtts.total_trx_quantity, 0)::float8 ELSE '--' END AS total_dyeing_quantity,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(slps.coloring_production_quantity, 0)::float8 ELSE '--' END as total_coloring_quantity,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(slps.coloring_production_quantity_weight, 0)::float8 ELSE '--' END as total_coloring_quantity_weight,
	...
	try {
		...
		const query = sql`
            ...
            SELECT
                ...
                CASE WHEN tbm.batch_rank = 1 THEN toe.quantity::float8 ELSE '--' END as quantity,
                CASE WHEN tbm.batch_rank = 1 THEN toe.party_price::float8 ELSE '--' END as party_price,
                CASE WHEN tbm.batch_rank = 1 THEN toe.company_price::float8 ELSE '--' END as company_price,
                toe.swatch_approval_received_date as swatch_approval_date,
                toe.swatch_approval_received as swatch_approval_received,
                recipe.name as recipe_name,
                CASE WHEN tbm.batch_rank = 1 AND toe.recipe_uuid IS NULL THEN toe.quantity::float8 ELSE '--' END as not_approved_quantity,
                CASE WHEN tbm.batch_rank = 1 AND toe.recipe_uuid IS NOT NULL THEN toe.quantity::float8 ELSE '--' END as approved_quantity,
                '"' || count_length.count as count,
                count_length.length,
                '"' || count_length.count || ' - ' || count_length.length as count_length_name,
                fto.uuid as order_info_uuid,
                CASE WHEN tbm.batch_rank = 1 THEN toe.delivered::float8 ELSE '--' END as delivered,
                CASE WHEN tbm.batch_rank = 1 THEN toe.warehouse::float8 ELSE '--' END as warehouse,
                CASE WHEN tbm.batch_rank = 1 THEN (toe.quantity::float8 - toe.delivered::float8) ELSE '--' END as balance_quantity,
                ...
                CASE WHEN tbm.batch_rank = 1 THEN COALESCE(bps.yarn_quantity, 0)::float8 ELSE '--' END as total_yarn_quantity,
                CASE WHEN tbm.batch_rank = 1 THEN (toe.quantity * count_length.max_weight)::float8 ELSE '--' END as total_expected_weight,
                tbm.batch_uuid,
                tbm.batch_number,
                tbm.production_date,
