	...
	try {
		...
		const query = sql`
            ...
            SELECT
                ...
                CASE WHEN dbm.batch_rank = 1 THEN oe.quantity::float8 ELSE 0 END as quantity,
                CASE WHEN dbm.batch_rank = 1 THEN oe.party_price::float8 ELSE 0 END as party_price,
                CASE WHEN dbm.batch_rank = 1 THEN oe.company_price::float8 ELSE 0 END as company_price,
                CASE WHEN sfg.recipe_uuid IS NOT NULL THEN oe.swatch_approval_received_date END as swatch_approval_date,
                CASE WHEN dbm.batch_rank = 1 AND sfg.recipe_uuid IS NULL THEN oe.quantity::float8 ELSE 0 END as not_approved_quantity,
                CASE WHEN dbm.batch_rank = 1 AND sfg.recipe_uuid IS NOT NULL THEN oe.quantity::float8 ELSE 0 END as approved_quantity,
                sfg.recipe_uuid,
                recipe.name as recipe_name,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(oe.quantity, 0)::float8 ELSE 0 END as total_quantity,
                CASE 
                ...
                CASE WHEN dbm.batch_rank = 1 THEN sfg.delivered::float8 ELSE 0 END as delivered,
                CASE WHEN dbm.batch_rank = 1 THEN sfg.warehouse::float8 ELSE 0 END as warehouse,
                CASE WHEN dbm.batch_rank = 1 THEN (oe.quantity::float8 - sfg.delivered::float8) ELSE 0 END as balance_quantity,
                fo.order_type,
                fo.is_inch,
                CASE
                    ...
                    ELSE 'Cm'
                END as unit,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(sps.finishing_quantity, 0)::float8 ELSE 0 END as total_finishing_quantity,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(dtts.total_trx_quantity, 0)::float8 ELSE 0 END AS total_dyeing_quantity,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(slps.coloring_production_quantity, 0)::float8 ELSE 0 END as total_coloring_quantity,
                CASE WHEN dbm.batch_rank = 1 THEN COALESCE(slps.coloring_production_quantity_weight, 0)::float8 ELSE 0 END as total_coloring_quantity_weight,
                ...
                dbm.production_date,
                COALESCE(dbm.total_quantity, 0)::float8,
                COALESCE(dbm.total_production_quantity, 0)::float8,
                CASE WHEN dbm.batch_rank = 1 THEN dbm.received END as received,
                dbm.dyeing_machine,
                dbm.batch_created_at,
                COALESCE(dbm.expected_kg, 0) as batch_expected_kg,
                dbm.batch_type,
	...
	try {
		...
		const query = sql`
            ...
            SELECT
                ...
                CASE WHEN tbm.batch_rank = 1 THEN toe.quantity::float8 ELSE 0 END as quantity,
                CASE WHEN tbm.batch_rank = 1 THEN toe.party_price::float8 ELSE 0 END as party_price,
                CASE WHEN tbm.batch_rank = 1 THEN toe.company_price::float8 ELSE 0 END as company_price,
                toe.swatch_approval_received_date as swatch_approval_date,
                recipe.name as recipe_name,
                CASE WHEN tbm.batch_rank = 1 AND toe.recipe_uuid IS NULL THEN toe.quantity::float8 ELSE 0 END as not_approved_quantity,
                CASE WHEN tbm.batch_rank = 1 AND toe.recipe_uuid IS NOT NULL THEN toe.quantity::float8 ELSE 0 END as approved_quantity,
                '"' || count_length.count as count,
                count_length.length,
                '"' || count_length.count || ' - ' || count_length.length as count_length_name,
                fto.uuid as order_info_uuid,
                CASE WHEN tbm.batch_rank = 1 THEN toe.delivered::float8 ELSE 0 END as delivered,
                CASE WHEN tbm.batch_rank = 1 THEN toe.warehouse::float8 ELSE 0 END as warehouse,
                ...
                tbm.batch_number,
                tbm.production_date,
                COALESCE(tbm.total_quantity, 0)::float8,
                COALESCE(tbm.yarn_issued, 0)::float8,
                tbm.is_drying_complete,
                tbm.machine,
                tbm.batch_created_at,
                COALESCE(tbm.expected_kg, 0) as batch_expected_kg,
                tbm.batch_type,
                COALESCE(tbm.damaged_quantity, 0) as damaged_quantity,
