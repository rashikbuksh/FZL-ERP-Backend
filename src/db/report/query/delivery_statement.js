	...
	try {
		...
		const queryHeader = sql`
         WITH opening_all_sum AS (
                SELECT 
                    ...
                    coalesce(
                        SUM(
                            CASE WHEN lower(vodf.end_type_name) LIKE '%close end%' THEN vpl.quantity ::float8 ELSE 0 END
                        ), 
                        0
                    )::float8 AS total_close_end_quantity, 
                    coalesce(
                        SUM(
                            CASE WHEN lower(vodf.end_type_name) LIKE '%open end%' THEN vpl.quantity ::float8 ELSE 0 END
                        ), 
                        0
		...
		const zipperQuery = sql`
                SELECT 
                    ...
                    CONCAT('ZC', to_char(ch.created_at, 'YY'), '-', (ch.id::text)) AS challan_number,
                ...
                LEFT JOIN (
                    ...
                    FROM 
                        commercial.pi_cash
                    GROUP BY
                        jsonb_array_elements_text(order_info_uuids::jsonb)
		...
		const threadQuery = sql`
             SELECT 
                    ...
                    CONCAT('ST', CASE WHEN toi.is_sample = 1 THEN 'S' ELSE '' END, TO_CHAR(toi.created_at, 'YY'), '-', (toi.id::text), CASE WHEN toi.is_cash = 0 THEN '(PI)' ELSE '(CASH)' END) as order_number_with_cash,
                    ...
                    CONCAT('PL', to_char(pl.created_at, 'YY-MM'), '-', pl.id::text) AS packing_number,
                    DATE(pl.created_at) as packing_list_created_at,
                    CONCAT('TC', to_char(ch.created_at, 'YY'), '-', (ch.id::text)) AS challan_number,
                    DATE(ch.created_at) as challan_created_at,
