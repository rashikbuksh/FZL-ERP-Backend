...
export async function threadProductionStatusOrderWise(req, res, next) {
	...
	try {
		...
		if (status === 'completed') {
			query.append(
				sql` AND order_entry.quantity = coalesce(thread_challan_sum.total_delivery_delivered_quantity,0) + coalesce(thread_challan_sum.total_delivery_balance_quantity,0) AND order_info.is_sample = 0 `
			);
			...
			query.append(
				sql` AND order_entry.quantity = coalesce(thread_challan_sum.total_delivery_delivered_quantity,0) + coalesce(thread_challan_sum.total_delivery_balance_quantity,0) AND order_info.is_sample = 1 `
			);
