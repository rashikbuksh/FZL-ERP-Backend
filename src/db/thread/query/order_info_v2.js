...
export async function insert(req, res, next) {
	...
	const resultPromise = db
		...
		.returning({
			insertedId: sql`concat('ST', CASE WHEN order_info.is_sample = 1 THEN 'S' ELSE '' END, to_char(order_info.created_at, 'YY'), '-', order_info.id::text)`,
		});
...
export async function selectAll(req, res, next) {
	const query = sql`
		SELECT 
			...
			order_info.is_sample::boolean,
			order_info.is_bill::boolean,
			order_info.is_cash::boolean,
			order_info.delivery_date::timestamp,
			order_info.created_by,
			hr.users.name AS created_by_name,
			order_info.created_at::timestamp,
			order_info.updated_at::timestamp,
			order_info.remarks,
...
export async function selectThreadSwatch(req, res, next) {
	const resultPromise = db
		.select({
			...
			id: order_info.id,
			order_number: sql`concat('ST', CASE WHEN order_info.is_sample = 1 THEN 'S' ELSE '' END, to_char(order_info.created_at, 'YY'), '-', order_info.id::text)`,
