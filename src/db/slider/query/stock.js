...
export async function selectStockByFromSection(req, res, next) {
	...
	const query = sql`
		...
		LEFT JOIN (
			SELECT jsonb_agg(jsonb_build_object('label', oe.style, 'value', fbe.uuid, 'given_quantity', fbe.finishing_prod, 'left_quantity', (fbe.quantity - fbe.finishing_prod))) as style_object, stock.uuid
			FROM zipper.sfg
	...
	// ! Slider Where condition finishing_batch.is_completed = false
