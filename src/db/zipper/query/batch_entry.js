...
export async function getOrderDetailsForBatchEntry(req, res, next) {
	...
	const query = sql`
		...
		WHERE
			sfg.recipe_uuid IS NOT NULL AND coalesce(oe.quantity,0) - coalesce(be_given.given_quantity,0) > 0 AND CASE WHEN lower(vodf.item_name) = 'nylon' THEN vodf.nylon_stopper = tcr.nylon_stopper_uuid ELSE TRUE END vodf.tap_coil_uuid IS NOT NULL`;
