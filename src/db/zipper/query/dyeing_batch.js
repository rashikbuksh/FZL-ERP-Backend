...
export async function insert(req, res, next) {
	...
	try {
		const data = await dyeingBatchPromise;
...
export async function selectAll(req, res, next) {
	const query = sql`
		SELECT 
			dyeing_batch.uuid,
			dyeing_batch.id,
			concat('B', to_char(dyeing_batch.created_at, 'YY'), '-', LPAD(dyeing_batch.id::text, 4, '0')) as dyeing_batch_id,
...
export async function selectBatchDetailsByBatchUuid(req, res, next) {
	...
	try {
		...
		const response = {
			...
			batch_entry: dyeing_batch_entry?.data?.data || [],
		};

		const toast = {
			status: 200,
			type: 'select',
			msg: 'Dyeing Batch Details Full',
		};
