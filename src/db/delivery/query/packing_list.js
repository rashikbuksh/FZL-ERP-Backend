...
export async function selectAllOrderForPackingList(req, res, next) {
	...
	if (item_for == 'zipper' || item_for == 'sample_zipper') {
		...
		SELECT 
			...
			LEAST(oe.quantity::float8 - sfg.warehouse::float8 - sfg.delivered::float8, sfg.finishing_prod::float8) as max_quantity,
			0 as quantity,
			0 as poli_quantity,
			0 as short_quantity,
		...
		SELECT 
			...
			LEAST(toe.quantity::float8 - toe.warehouse::float8 - toe.delivered::float8, total_production_quantity.total_production_quantity::float8) as max_quantity,
			0 as quantity,
			0 as poli_quantity,
			0 as short_quantity,
			0 as reject_quantity,
