...
export async function selectAll(req, res, next) {
	const quantity_returnPromise = db
		.select({
			...
			color_ref: sql`CASE WHEN quantity_return.order_entry_uuid IS NOT NULL THEN zipper.order_entry.color_ref ELSE thread_order_entry.color_ref END`,
...
export async function selectOrderEntryFullByOrderInfoUuid(req, res, next) {
	...
	const orderEntryPromise = db
		.select({
			...
			color_ref: zipperSchema.order_entry.color_ref,
